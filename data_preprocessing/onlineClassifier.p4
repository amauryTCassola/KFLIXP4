...
table tableClassifier {
	...
	default_action = actionClassify(0,0,0,0,0,0,0,0);
}
action actionCalcPktCountDists(
	...
	bit<TIMESTAMP_WIDTH> clus4Dist;
	registerDistances.read(clus4Dist, 4);
	clus4Dist = clus4Dist + (bit<TIMESTAMP_WIDTH>)((normalized_feature - cluster4)*(normalized_feature - cluster4));
	registerDistances.write(4, clus4Dist);


	bit<TIMESTAMP_WIDTH> clus5Dist;
	registerDistances.read(clus5Dist, 5);
	clus5Dist = clus5Dist + (bit<TIMESTAMP_WIDTH>)((normalized_feature - cluster5)*(normalized_feature - cluster5));
	registerDistances.write(5, clus5Dist);


	bit<TIMESTAMP_WIDTH> clus6Dist;
	registerDistances.read(clus6Dist, 6);
	clus6Dist = clus6Dist + (bit<TIMESTAMP_WIDTH>)((normalized_feature - cluster6)*(normalized_feature - cluster6));
	registerDistances.write(6, clus6Dist);


	bit<TIMESTAMP_WIDTH> clus7Dist;
	registerDistances.read(clus7Dist, 7);
	clus7Dist = clus7Dist + (bit<TIMESTAMP_WIDTH>)((normalized_feature - cluster7)*(normalized_feature - cluster7));
